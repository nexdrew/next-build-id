...
module.exports = function nextBuildId (opts) {
  ...
  return resolveInputDir(opts.dir)
    ...
    .then(files => {
      ...
      return determineBuildTag(opts.tag, result.inputDir)
    })
    .then(tag => {
      result.tag = tag || null
...
function determineBuildTag (tag, inputDir) {
  ...
  return new Promise((resolve, reject) => {
    ...
    cp.execFile('git', [`--git-dir=${path.join(dir, '.git')}`, `--work-tree=${dir}`, 'describe', '--tags'], (err, stdout, stderr) => {
      if (err) return reject(err)
      if (stderr) return reject(String(stderr).trim())
      if (stdout) return resolve(String(stdout).trim())
      reject(`No output from command: git --git-dir=${path.join(dir, '.git')} --work-tree=${dir} describe --tags`)
    })
  })
}
